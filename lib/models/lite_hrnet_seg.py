...
from deepvac.backbones import Conv2dBNReLU, Conv2dBN, Concat
...
class SpatialWeighting(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = Conv2dBNReLU(
            in_planes = channels,
            out_planes = int(channels / ratio),
            kernel_size = 1,
            stride = 1)

        self.conv2 = Conv2dBNReLU(
            in_planes = int(channels / ratio),
            out_planes = channels,
            kernel_size = 1,
            stride = 1)
...
class CrossResolutionWeighting(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = Conv2dBNReLU(
            in_planes = total_channel,
            out_planes = int(total_channel / ratio),
            kernel_size = 1,
            stride = 1)

        self.conv2 = Conv2dBNReLU(
            in_planes = int(total_channel / ratio),
            out_planes = total_channel,
            kernel_size = 1,
            stride = 1)
...
class Stem(nn.Module):
    ...
    def __init__(self,
        ...
        self.branch1 = nn.Sequential(
            Conv2dBN(
                in_planes = branch_channels,
                out_planes = branch_channels,
                kernel_size = 3,
                stride = 2,
                padding = 1,
            ...
            Conv2dBNReLU(
                in_planes = branch_channels,
                out_planes = inc_channels,
                kernel_size = 1,
                stride = 1,
                padding = 0),
        )

        self.expand_conv = Conv2dBNReLU(
            in_planes = branch_channels,
        ...
        self.linear_conv = Conv2dBNReLU(
            in_planes = mid_channels,
            out_planes = branch_channels if stem_channels == self.out_channels else stem_channels,
            kernel_size = 1,
            stride = 1,
            padding = 0)
            ...
            if i != num_branchs - 1:
                projects.append(
                    nn.Sequential(
                        ...
                        Conv2dBNReLU(
                            in_planes = self.in_channels[i],
                            out_planes = self.in_channels[i+1],
                            kernel_size = 1,
                            stride = 1,
                            padding = 0)))
            else:
...
class ShuffleUnit(nn.Module):
    ...
    def __init__(self,
        ...
        self.branch2 = nn.Sequential(
            Conv2dBNReLU(
                in_planes = in_channels if (self.stride > 1) else branch_features,
                out_planes = branch_features,
                kernel_size = 1,
                stride = 1,
                padding = 0),
            Conv2dBN(
                in_planes = branch_features,
                out_planes = branch_features,
                kernel_size = 3,
                stride = self.stride,
                padding = 1,
            ...
            for j in range(num_branches):
                if j > i:
                    fuse_layer.append(
                        nn.Sequential(
                            Conv2dBN(
                                in_planes = in_channels[j],
                                out_planes = in_channels[i],
                                kernel_size = 1,
                                stride = 1,
                                padding = 0),
                            nn.Upsample(
            ...
            if i < num_branches_pre:
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    transition_layers.append(
                        nn.Sequential(
                            Conv2dBN(
                                in_planes = num_channels_pre_layer[i],
                                out_planes = num_channels_pre_layer[i],
                                kernel_size = 3,
                                stride = 1,
                                padding = 1,
                            ...
                            Conv2dBNReLU(
                                in_planes = num_channels_pre_layer[i],
                                out_planes = num_channels_cur_layer[i],
                                kernel_size = 1,
                                stride = 1,
                                padding = 0)))
                else:
...
class SegLiteHRNet(nn.Module):
    def __init__(self, num_classes=4):
        super(SegLiteHRNet, self).__init__()
        extra=dict(
        ...
        self.backbone = LiteHRNet(extra, in_channels = 3)

        out = list(extra["stages_spec"]["num_channels"][-1])
        conv_out = 40
        ...
        self.header = nn.Sequential(
            nn.Conv2d(conv_out*4, conv_out, kernel_size=3, stride=1, padding=1),
            nn.Conv2d(conv_out, num_classes, 1, 1))
